...
def train(model, args, test=False):
    ...
    for i in pbar:
        ...
        if args.training.task == "matrix_chain":
            ...
            loss, output = train_step(model, xs.cuda(), ys.cuda(), optimizer, loss_func, print_loss=False, block_size=block_size, n=data_sampler.n)
        elif args.training.task == "matrix_chain_vector":
            block_size = 2 
            loss, output = train_step(model, xs.cuda(), ys.cuda(), optimizer, loss_func, print_loss=False, block_size=block_size, n=1)#data_sampler.n)
        else:
            loss, output = train_step(model, xs.cuda(), ys.cuda(), optimizer, loss_func, print_loss=False, block_size=block_size, n=1)#data_sampler.n)
