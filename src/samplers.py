...
class TableConnectivityFixedEmbeddingSampler(DataSampler):
    """
    Sampler for TableConnectivity task with fixed embeddings across all prompts.
    
    Unlike TableConnectivitySampler which generates random embeddings at initialization,
    this sampler uses a fixed seed to generate embeddings, ensuring that all training
    samples use the same table and column embeddings.
    """
    
    def __init__(self, n_dims, V=5, C=3, rho=0.5, bias=None, scale=None, embedding_seed=42, **kwargs):
        """
        Args:
            n_dims: Total number of dimensions
            V: Number of tables
            ...
            bias: Optional bias for the features
            scale: Optional scaling transformation
            embedding_seed: Seed for generating fixed embeddings (default: 42)
        """
        super().__init__(n_dims)
        self.V = V
        self.C = C
        ...
        # Generate FIXED random vectors using a fixed seed
        # This ensures all instances of this sampler use the same embeddings
    ...
    def sample_xs(self, n_points, b_size, n_dims_truncated=None, seeds=None):
        """
        Sample input data for TableConnectivity task with fixed embeddings.
